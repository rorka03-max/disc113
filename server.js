...
app.get('/api/channels', authenticateToken, async (req, res) => {
    try {
        // Return all text channels for now (no servers filtering implemented yet)
        // Minimal query: reuse db directly
        const sql = 'SELECT id, name, type FROM channels ORDER BY id ASC';
        require('./database').db.all(sql, [], (err, rows) => {
            if (err) return res.status(500).json({ error: 'Failed to get channels' });
            res.json(rows);
        });
    } catch (error) {
...
io.on('connection', async (socket) => {
    ...
    // Join/leave text channels (per-channel rooms)
    socket.on('join-text-channel', (channelId) => {
        try {
            if (channelId == null) return;
            socket.join(`text-${channelId}`);
        } catch (e) {
            console.error('join-text-channel error:', e);
        }
    });
    ...
    socket.on('leave-text-channel', (channelId) => {
        try {
            ...
            console.error('leave-text-channel error:', e);
        }
    });
    ...
    socket.on('send-message', async (messageData) => {
        try {
            ...
            io.to(`text-${channelId}`).emit('new-message', {
                channelId,
